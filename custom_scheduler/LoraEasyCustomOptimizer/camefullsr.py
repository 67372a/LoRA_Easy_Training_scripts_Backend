...
class CAMEFullSR(BaseOptimizer):
    ...
    def __init__(
        self,
        params: PARAMETERS,
        lr: float = 7e-5, #Original default 2e-4
        betas: BETAS = (0.9, 0.999, 0.9999),
        weight_decay: float = 0.001, #Original default 0.0
    ...
    def step(self, closure: CLOSURE = None) -> LOSS:
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if factored:
                    if p.dtype in {torch.float16, torch.bfloat16}:
                        exp_avg_sq_row, exp_avg_sq_col = state['exp_avg_sq_row'].to(torch.float32), state['exp_avg_sq_col'].to(torch.float32)
                    else:
                    ...
                    if p.dtype in {torch.float16, torch.bfloat16}:
                        copy_stochastic_(state["exp_avg_sq_row"], exp_avg_sq_row)
                        copy_stochastic_(state["exp_avg_sq_col"], exp_avg_sq_col)
                else:
                    if p.dtype in {torch.float16, torch.bfloat16}:
                ...
                if group['ams_bound']:
                    if p.dtype in {torch.float16, torch.bfloat16}:
                        exp_avg_sq_hat = state["exp_avg_sq_hat"].to(torch.float32)
                    else:
                ...
                if factored:
                    if p.dtype in {torch.float16, torch.bfloat16}:
                        exp_avg_res_row, exp_avg_res_col = state['exp_avg_res_row'].to(torch.float32), state['exp_avg_res_col'].to(torch.float32)
                    else:
                    ...
                    if p.dtype in {torch.float16, torch.bfloat16}:
                        copy_stochastic_(state['exp_avg_res_row'], exp_avg_res_row)
                        copy_stochastic_(state['exp_avg_res_col'], exp_avg_res_col)
